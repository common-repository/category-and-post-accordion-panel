<?php if ( ! defined( 'ABSPATH' ) ) exit;   $vcode = $this->_config["vcode"];  ?>
 ...
 <div id="postaccordionpanel" style="width:<?php echo $this->_config["tp_widget_width"]; ?>"  class="pane_style_3 <?php echo ( ( trim( $this->_config["display_title_over_image"] ) == "yes" ) ? "disp_title_over_img" : "" ); ?>">
			...
			if( count( $_category_res ) > 0 ) {
				foreach($_category_res as $_category){ 
					...
					<div class="item-pst-list">
						<div class="pst-item"  onmouseout="apcp_cat_tab_ms_out( this )" onmouseover="apcp_cat_tab_ms_hover( this )" id="<?php echo $vcode.'-'.$_category_id; ?>" onclick="APCP_fillPosts( this.id, '<?php echo esc_js($_category_id ); ?>', request_obj_<?php echo esc_js( $vcode ); ?>, 1 )"  style="color:<?php echo esc_attr($this->_config["category_tab_text_color"] ); ?>;background-color:<?php echo esc_attr( $this->_config["category_tab_background_color"] ); ?>;" >
