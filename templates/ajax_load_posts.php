<?php if ( ! defined( 'ABSPATH' ) ) exit; 
	...
	if( $_total_posts <= 0 ) {
		?><div class="ik-post-no-items"><?php echo __( 'No posts found.', 'postaccordionpanel' ); ?></div><?php
		die();
	}  
	...
	foreach ( $post_list as $_post ) { 
		...
		<div class='ik-post-item pid-<?php echo esc_attr( $_post->post_id ); ?>'> 
			<div class='ik-post-image' onmouseout="apcp_pr_item_image_mouseout(this)" onmouseover="apcp_pr_item_image_mousehover(this)">
	...
	if( $_total_posts > sanitize_text_field( $params["number_of_post_display"] ) ) { ?>
			<div class="clr"></div>
			<div class='ik-post-load-more'  align="center" onclick='APCP_loadMorePosts( <?php echo esc_js( $category_id ); ?>, "<?php echo esc_js( $_limit_start+$_limit_end ); ?>", "<?php echo esc_js( $params["vcode"]."-".$category_id ); ?>", "<?php echo esc_js( $_total_posts ); ?>", request_obj_<?php echo esc_js( $params["vcode"] ); ?> )'>
				<?php echo __('Load More', 'postaccordionpanel' ); ?>
			</div>
		<?php  
	} else {
