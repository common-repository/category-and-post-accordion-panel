...
if ( ! class_exists( 'categoryPostAccordionPanelWidget' ) ) { 
	class categoryPostAccordionPanelWidget extends categoryPostAccordionPanelLib {
		...
		*/ 
		public function init() {
			...
			add_shortcode( 'postaccordionpanel', array( &$this, 'categoryPostAccordionPanel' ) ); 
		...
		*/
		public function categoryPostAccordionPanel( $params = array() ) { 	
		
			$postaccordionpanel_id = $params["id"]; 
			$apcp_shortcode = get_post_meta( $postaccordionpanel_id ); 
			
			foreach ( $apcp_shortcode as $sc_key => $sc_val ) {			
				$apcp_shortcode[$sc_key] = $sc_val[0];			
			} 
			
			if(!isset($apcp_shortcode["number_of_post_display"]))	
				$apcp_shortcode["number_of_post_display"] = 0;
			...
			*/
			...
			require( $this->getCategoryPostAccordionPanelTemplate( "template_" . $this->_config["template"] . ".php" ) ); 
		...
		*/	
		public function getMorePosts() {
			...
			*/
			...
			$_limit_end = ( isset( $_REQUEST["number_of_post_display"])?esc_attr( $_REQUEST["number_of_post_display"] ):apcp_number_of_post_display ); 
			...
			*/	
			...
			require( $this->getCategoryPostAccordionPanelTemplate( 'ajax_load_more_posts.php' ) );	
		...
		*/
		public function getPosts() {
		   ...
		   require( $this->getCategoryPostAccordionPanelTemplate( 'ajax_load_posts.php' ) );	
...
new categoryPostAccordionPanelWidget();